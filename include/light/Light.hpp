...
struct light_struct {
  light_struct()
      : light_struct(glm::vec3(0.f), glm::vec3(0.f), glm::vec3(0, 1, 0)) {}
  ...
  light_struct(const glm::vec3 &pos, const glm::vec3 &intense,
      ...
      : position(pos), intensity(intense), axis(_axis), angle(_angle),
        translation(_translation), scale(_scale) {}

  glm::vec3 position;
  glm::vec3 intensity;

  const glm::mat4x4 &updateModelMatrix() {
  ...
  [[nodiscard]] const glm::mat4x4 &getModelMatrix() {
