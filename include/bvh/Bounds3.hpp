...
namespace SoftRasterizer {
          struct Bounds3 {
                    ...
                    Bounds3(const glm::vec3& p1, const glm::vec3& p2)
                              :min(glm::vec3(std::fmin(p1.x, p2.x), std::fmin(p1.y, p2.y), std::fmin(p1.z, p2.z)))
                              , max(glm::vec3(std::fmax(p1.x, p2.x), std::fmax(p1.y, p2.y), std::fmax(p1.z, p2.z)))
                    {
                    }
                    ...
                    Bounds3 intersect(const Bounds3& b) {
                              return Bounds3(
                                        glm::vec3(std::fmax(this->min.x, b.min.x), std::fmax(this->min.y, b.min.y), std::fmax(this->min.z, b.min.z)),
                                        glm::vec3(std::fmin(this->max.x, b.max.x), std::fmin(this->max.y, b.max.y), std::fmin(this->max.z, b.max.z)));
                    }
