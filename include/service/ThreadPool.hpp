...
namespace SoftRasterizer {
          ...
          public:
                    ...
                    std::future<typename std::invoke_result_t<F, Args...>> /*Ret Value*/
                              ...
                              if (m_stopFlag) { throw std::runtime_error("Thread pool is stopped."); }

                              auto task = std::make_shared<FuncType>(std::bind(std::forward<F>(f), std::forward<Args>(args)...));
          ...
          protected:
                    ...
                    void worker(std::size_t thread_id);      //Working Thread, Used For Create Thread Pool
