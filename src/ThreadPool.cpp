...
SoftRasterizer::ThreadPool::ThreadPool(std::size_t number)
    : m_stopFlag(false), m_threads(number <= 1 ? 2 : number) {
  try {
    // Start Thread Pool
    initialize();
...
SoftRasterizer::ThreadPool::~ThreadPool() { terminate(); }

/*Start ThreadPool*/
void SoftRasterizer::ThreadPool::initialize() {
...
void SoftRasterizer::ThreadPool::terminate() {
  /*Stop Flag = True*/
...
void SoftRasterizer::ThreadPool::worker(std::size_t thread_id) {
  while (true) {
