...
SoftRasterizer::RenderingPipeline::RenderingPipeline(
    ...
    : m_width(width), m_height(height), m_channels(numbers) /*set to three*/
      ...
      inf(simde_mm256_set1_ps(
          std::numeric_limits<float>::infinity())) /*SIMD inf*/
      ,
      one(simde_mm256_set1_ps(1.0f)) /*SIMD one*/
      ,
      zero(simde_mm256_set1_ps(0.0f)), UNROLLING_FACTOR(8) {
...
SoftRasterizer::RenderingPipeline::barycentric(
  ...
  simde__m256 areaABC =
      simde_mm256_fmsub_ps(ABx, ACy, simde_mm256_mul_ps(ACx, ABy)); // AB x AC
  simde__m256 inverse = simde_mm256_rcp_ps(areaABC);

  // Compute area of triangle PBC (cross product of PB ¡Á PC)
  simde__m256 areaPBC =
      simde_mm256_fmsub_ps(PBx, PCy, simde_mm256_mul_ps(PCx, PBy)); // PB ¡Á PC

  // Compute area of triangle PCA (cross product of PC ¡Á PA)
  simde__m256 areaPCA =
      simde_mm256_fmsub_ps(PCx, PAy, simde_mm256_mul_ps(PAx, PCy)); // PC ¡Á PA
