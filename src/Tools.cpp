...
Eigen::Vector4f 
SoftRasterizer::Tools::to_vec4(const Eigen::Vector3f& v3, float w) {
...
SoftRasterizer::Tools::calculateRotationMatrix(const Eigen::Vector3f& axis,
                                                                            float angle) 
{
          spdlog::info("Calculating rotation matrix for angle: {} and axis: ({}, {}, {})", angle, axis.x(), axis.y(), axis.z());
          ...
          const Eigen::Vector3f origin = Eigen::Vector3f(0.f, 0.f, 0.f);         //origin point(0, 0, 0)
          ...
          transform_to_matrix <<
                    1, 0, 0, transform_to.x(),
                    0, 1, 0, transform_to.y(),
                    0, 0, 1, transform_to.z(),
                    0, 0, 0, 1;
          ...
          Rodrigues << 0, -normalize_axis.z(), normalize_axis.y(),
                    normalize_axis.z(), 0, -normalize_axis.x(),
                    -normalize_axis.y(), normalize_axis.x(), 0;

          //cos E + (1-cos)(n * n^T)+sin * Rodrigues
          Rodrigues = cos * Eigen::Matrix3f::Identity() + (1.0f - cos) * normalize_axis * normalize_axis.transpose() + sin * Rodrigues;

          rotationMatrix4.block<3, 3>(0, 0) = Rodrigues;

          return move_back_matrix * rotationMatrix4 * transform_to_matrix * rotationMatrix;
}
 ...
 *
 * @param translation A 3D vector representing the translation (position) of the object.
 ...
 *
 * @return A 4x4 model matrix (Eigen::Matrix4f) representing the combined transformations.
 */
Eigen::Matrix4f
 ...
 * @param up The up direction vector for the camera, which defines the orientation.
 *
 * @return A 4x4 view matrix (Eigen::Matrix4f) representing the camera's view transformation.
 */
Eigen::Matrix4f 
                                                                      ...
                                                                      const Eigen::Vector3f& up) 
{
          ...
          const Eigen::Vector3f look_at = (eye - center).normalized();    //look at normalize
 ...
 * projection is defined by the left, right, bottom, top, zNear, and zFar parameters.
 *
 ...
 */
Eigen::Matrix4f 
SoftRasterizer::Tools::calculateOrthoMatrix(float left, 
                                                                        float right,
                                                                        float bottom, 
                                                                        float top, 
                                                                        float zNear, 
 ...
 */
Eigen::Matrix4f 
SoftRasterizer::Tools::calculateProjectionMatrix(float fovy, 
                                                                                float aspect,
                                                                                float zNear, 
                                                                                float zFar) 
{
          ...
          Eigen::Matrix4f Ortho = calculateOrthoMatrix(left, right, bottom, top, zNear, zFar);
