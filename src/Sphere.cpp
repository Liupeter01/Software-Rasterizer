...
SoftRasterizer::Bounds3 SoftRasterizer::Sphere::getBounds() {
  Bounds3 ret;
  ret.min = glm::vec3(vert[0].position.x - radius, vert[0].position.y - radius, vert[0].position.z - radius);
  ret.max = glm::vec3(vert[0].position.x + radius, vert[0].position.y + radius, vert[0].position.z + radius);
  return ret;
}
