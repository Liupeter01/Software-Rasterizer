...
float SoftRasterizer::Sphere::getSquare() const {
          return square;
}
...
bool SoftRasterizer::Sphere::intersect(const Ray& ray) {
          ...
          auto b = 2.f * ray.direction.x * L.x + ray.direction.y * L.y + ray.direction.z * L.z;
...
bool SoftRasterizer::Sphere::intersect(const Ray& ray, float& tNear) {
          ...
          if (!res) {         //res == 0
                    ...
                    float q = (b > 0) ? -0.5 * (b + std::sqrt(res)) : -0.5 * (b - std::sqrt(res));
                    auto x0 = q / a;
                    auto x1 = c / q;

                    if (x0 < x1 && x0 >0) { tNear = x0; }
                    else if (x1 < x0 && x1 > 0) { tNear = x1; }
                    else {
                              return false;
                    }
          }
